...
- [Multipliers \& multiplier effects](#multipliers--multiplier-effects)
  ...
  - [Steps for the multiplier effects](#steps-for-the-multiplier-effects)
    - [**1. Variation in output $y$**](#1-variation-in-output-y)
    - [**2. Variation in disposable income $y^D$**](#2-variation-in-disposable-income-yd)
    - [**3. variation in budget deficit $BD$**](#3-variation-in-budget-deficit-bd)
...
3. assume a change in <ins>transfers</ins> $Tr$
    ...
    > we know from [government spending multiplier](#1-variation-in-output) that $\Delta y^D = \dfrac{1}{1 - c \cdot (1 - t)} \cdot c \cdot \Delta Tr$
...
4. assume a change in <ins>autonomous taxes</ins> $T_0$  
    ...
    > we know from [autonomous tax multiplier](#1-variation-in-output) that $\Delta y = (-c) \cdot \alpha \cdot \Delta T$  
    ...
    $\Delta y^D = -\alpha _G \cdot \Delta T_0$  
    $\Rightarrow m \cdot \Delta y^D_{T_0} = \dfrac{\Delta y^D}{\Delta T_0} = - \alpha _G$  
...
2. assume a change in <ins>transfers</ins> $Tr$  
    ...
    $m \cdot BD_{Tr} = \dfrac{\Delta BD}{\Delta Tr} = 1 - t \cdot c \cdot \alpha _G$  