...
<ins>[**Seminar PDF**](./seminar%20pdfs/S08.%20ISLM.pdf)
...
To identify the macroeconomic equilibrium we first extract $r$ from the $LM$ curve and replace it in the $IS$ curve as follows:  
...
Determine:  
&emsp; (a) the $IS$ and $LM$ equations bot analogically and numerically  
...
&emsp; (e) by how much should te money supply be modified to bring $GDP$ back to its original level, what will be the new interest rate?  
&emsp; Build the graphs to illustrate the equilibrium solution for (d) and (e)  

---

(a)  
&emsp; Analogically (they were computed in the seminar):  
&emsp; $IS: y = \dfrac{1}{1 - c \cdot (1 - t)} \cdot \left(C _0 - c \cdot T _0 + c \cdot Tr + I _0 + \overline{G} + \dfrac{b}{h} \cdot \dfrac{M}{p} \right)$  
&emsp; $LM: y = \dfrac{1}{k} \cdot \dfrac{M}{p} + \dfrac{h}{k} \cdot r$  
...
(b)  
&emsp; $r ^ * = \dfrac{k}{h} \cdot y ^ * - \dfrac{1}{h} \cdot \dfrac{M}{p}$  
> To calculate the equilibrium we use the ISLM equilibrium:  
...
&emsp; $\Delta r ^ * = \dfrac{k}{h} \cdot \Delta y = \dfrac{1}{10000} \cdot (-300) = -3\%$  
